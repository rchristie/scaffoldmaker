...
def get_bifurcation_triple_point(p1x, p1d, p2x, p2d, p3x, p3d):
    ...
    print("scaling", scaling12, scaling23, scaling31)
    ...
    # weight more heavily to larger derivatives
    mag_p1d = magnitude(p1d)
    mag_p2d = magnitude(p2d)
    mag_p3d = magnitude(p3d)
    weight1 = 1.0  # (mag_p1d + mag_p2d) ** 2
    weight2 = 1.0  # (mag_p2d + mag_p3d) ** 2
    weight3 = 1.0  # (mag_p3d + mag_p1d) ** 2
    totalWeight = weight1 + weight2 + weight3
    trx = [(trx1[c] * weight1 + trx2[c] * weight2 + trx3[c] * weight3) / totalWeight for c in range(3)]
...
def generateTubeBifurcationTree(networkMesh: NetworkMesh, region, coordinates, nodeIdentifier, elementIdentifier,
                                ...
                                showIntersectionCurves=True, showTrimSurfaces=False):
    """
