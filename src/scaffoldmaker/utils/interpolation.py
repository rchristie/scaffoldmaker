...
def getNearestLocationOnCurve(nx, nd1, targetx, loop=False, startLocation=None, instrument=False):
    ...
    for it in range(100):
        ...
        mag_dxi = abs(dxi)
        if instrument:
            print("iter", it, "location", location, "dxi", dxi, "deltax", deltax)
...
def getNearestLocationBetweenCurves(nx, nd1, ox, od1, nLoop=False, oLoop=False, startLocation=None, instrument=False):
    ...
    for it in range(100):
        ...
        otherLocation = getNearestLocationOnCurve(ox, od1, x, oLoop, otherLocation, instrument=(it == 2))[0]
        ...
        mag_r = magnitude(r)
        if instrument:
            print("iter", it, "pos", location, "other", otherLocation, "mag_r", mag_r)
        if mag_r < x_tol:
        ...
        if onOtherBoundary:
            ...
            # slope_factor = 1.0 + r_dot_n / mag_r  # GRC wrong
            u = mult(rTangent, slope_factor)
        mag_dxi = magnitude(u) / magnitude(d)
        if mag_dxi > MAX_MAG_DXI:
            mag_dxi = MAX_MAG_DXI
        dxi = mag_dxi
        if dot(u, d) < 0.0:
